...
class Augmenter(torch.nn.Module):
    def __init__(self, p, gan, gan_config_paths, gan_checkpoint_paths, classifier_checkpoint_paths, correct_threshold,
                 incorrect_threshold, device):
        super().__init__()
        self.p = p
        self.gan = gan
    ...
    def next_generator(self):
        ...
        self.gen = FilteredImageGenerator(self.gan_config_paths[self.current], self.gan_checkpoint_paths[self.current],
                                          self.classifier_checkpoint_paths[self.current], self.correct_threshold,
                                          self.incorrect_threshold, self.device)
...
class FilteredImageGenerator(torch.nn.Module):
    def __init__(self, config_path, checkpoint_path, classifier_path, correct_threshold, incorrect_threshold, device):
        super().__init__()
        self.config_path = config_path
        self.checkpoint_path = checkpoint_path
        self.classifier_path = classifier_path
        self.classifier = Classifier(device, classifier_path)
        self.features = None
        self.labels = None
        self.device = device
    ...
    def _filter_imgs(self, label):
        ...
        predictions, probabilities = self.classifier.classify(self.imgs[label])

        for idx, (pred, prob) in enumerate(zip(predictions, probabilities)):
            if pred == label and prob > self.correct_threshold:
                filtered.append(idx)
...
class Classifier:
    ...
    def classify(self, images):
        ...
        for batch in images:
            batch = torch.stack([preprocess(image) for image in batch], dim=0).to(self.device)
            with torch.no_grad():
                outputs = nn.functional.softmax(self.model(batch), dim=1)
...
if __name__ == '__main__':
    ...
    augmenter = Augmenter(p, gan, gan_configs, gan_checkpoints, classifier_checkpoints, correct_threshold,
                          incorrect_threshold, device)
