...
class MappingNetwork(torch.nn.Module):
    def __init__(
        ...
        features_list = [z_dim if z_dim else embed_features] + [layer_features] * (num_layers - 1) + [w_dim]

        if c_dim > 0:
            self.embed = FullyConnectedLayer(c_dim, embed_features)
            if z_dim:
                self.proj_embedd = FullyConnectedLayer(embed_features, features_list[0], activation=activation,
                                                       lr_multiplier=lr_multiplier)
                self.register_buffer("transition", torch.zeros([]))
    ...
    def forward(self, z, c, truncation_psi=1, truncation_cutoff=None, update_emas=False, skip_w_avg_update=False):
...
class DiscriminatorEpilogue(torch.nn.Module):
    def __init__(
        ...
        if cmap_dim:
            self.out_c = FullyConnectedLayer(in_channels, cmap_dim)
    ...
    def forward(self, x, img, cmap, force_fp32=False):
        ...
        if self.cmap_dim > 0:
            ...
            out_c = (out_c * cmap).sum(dim=1, keepdim=True) * (1 / np.sqrt(self.cmap_dim))
...
class Discriminator(torch.nn.Module):
    ...
    def forward(self, img, c, eval=False, adc_fake=False, update_emas=False, **block_kwargs):
        ...
        c = F.one_hot(c, self.num_classes)

        cmap = None
        if self.c_dim > 0:
            cmap = self.mapping(None, c)
        x, x_c = self.b4(x, img, cmap)
