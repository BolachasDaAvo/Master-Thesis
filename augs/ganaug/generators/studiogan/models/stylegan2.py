...
import augs.ganaug.generators.studiogan.utils.style_misc as misc
from augs.ganaug.generators.studiogan.utils.style_ops import bias_act, fma, upfirdn2d, conv2d_resample
...
class Generator(torch.nn.Module):
    ...
    def forward(self, z, c, eval=False, truncation_psi=1, truncation_cutoff=None, update_emas=False,
                **synthesis_kwargs):
...
class Discriminator(torch.nn.Module):
    def __init__(
        ...
        self.b4 = DiscriminatorEpilogue(channels_dict[4], cmap_dim=self.cmap_dim, resolution=4, **epilogue_kwargs,
                                        **common_kwargs)
        ...
        if self.d_cond_mtd == "AC":
            ...
            self.mapping = MappingNetwork(z_dim=0, c_dim=c_dim, w_dim=self.cmap_dim, num_ws=None, w_avg_beta=None,
                                          **mapping_kwargs)
        elif self.d_cond_mtd in ["2C", "D2DCE"]:
            self.linear2 = FullyConnectedLayer(channels_dict[4], d_embed_dim, bias=True)
            self.embedding = MappingNetwork(z_dim=0, c_dim=c_dim, w_dim=d_embed_dim, num_ws=None, w_avg_beta=None,
                                            num_layers=1, **mapping_kwargs)
        else:
            pass
        ...
        if self.aux_cls_type == "TAC":
            if self.d_cond_mtd == "AC":
                ...
                self.embedding_mi = MappingNetwork(z_dim=0, c_dim=c_dim, w_dim=d_embed_dim, num_ws=None,
                                                   w_avg_beta=None, num_layers=1, **mapping_kwargs)
            else:
        ...
        if self.MODEL.info_type in ["discrete", "both"]:
            ...
            self.info_discrete_linear = FullyConnectedLayer(in_features=channels_dict[4], out_features=out_features,
                                                            bias=False)
        if self.MODEL.info_type in ["continuous", "both"]:
            out_features = self.MODEL.info_num_conti_c
            self.info_conti_mu_linear = FullyConnectedLayer(in_features=channels_dict[4], out_features=out_features,
                                                            bias=False)
            self.info_conti_var_linear = FullyConnectedLayer(in_features=channels_dict[4], out_features=out_features,
                                                             bias=False)
